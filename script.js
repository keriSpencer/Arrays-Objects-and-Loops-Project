...
"property" and return the difference. This number is the amount lost by the
studio in making the film. Return the loss.
*/
function getLosses(){
  let loss;
...
/*
7. Iterate over "letterVals" and "numberVals". Concatenate the values from the
two arrays and store the new values in the "vals" array. Return "vals".
...
function interleave(){
  let vals = [];
  // Your answer here:
  for (let j = 0; j < letterVals.length; j++) {
    let combine = letterVals[j] + numberVals[j]
    vals.push(combine)
  }
  return vals
}
